...
export function FooterLink({ href, className, ...props }: { href: string } & Omit<ComponentProps<'a'>, 'href'>) {
  return (
    <li className={clsx('text-zinc-700 dark:text-zinc-400', className)}>
...
export function SocialLink({
  ...
  return (
    <Link
      ...
      target="_blank"
      aria-label={name}
      className={clsx('text-zinc-950 *:size-6 dark:text-white', className)}
      {...props}
    />
  )
}
...
export function NewsletterForm({
  ...
  return (
    <form className={clsx('flex max-w-sm flex-col gap-2', className)} {...props}>
      ...
      <div className="flex items-center border-b border-zinc-950/20 py-2 has-[input:focus]:border-zinc-950 dark:border-white/20 dark:has-[input:focus]:border-white">
        <input
          type="email"
          placeholder="Email"
          aria-label="Email"
          className="flex-1 text-zinc-950 focus:outline-hidden dark:text-white"
        />
        <button
          type="submit"
          aria-label="Subscribe"
          className="relative inline-flex size-7 items-center justify-center rounded-full after:absolute after:-inset-2 hover:bg-zinc-950/10 dark:hover:bg-white/10 after:pointer-fine:hidden"
        >
...
export function FooterWithNewsletterFormCategoriesAndSocialIcons({
  ...
  return (
    <footer className={clsx('pt-16', className)} {...props}>
      <div className="bg-zinc-950/2.5 py-16 text-zinc-950 dark:bg-white/5 dark:text-white">
        <Container className="flex flex-col gap-16">
          ...
          <div className="flex items-center justify-between gap-10 text-sm/7">
            <div className="text-zinc-600 dark:text-zinc-500">{fineprint}</div>
