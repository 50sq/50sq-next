...
import { GlobeBlock } from "./globe-block";
...
const cards = [
  {
    id: "upload",
    title: "Photo Upload",
    subtitle: "Batch upload your high-resolution photos",
    icon: Upload,
  ...
  },
  {
    ...
    subtitle: "Photos optimized for web viewing instantly",
    icon: Image,
...

  {
    ...
    subtitle: "Password protection, download controls, and expiry dates",
    icon: Shield,
        ...
        <div className="relative z-1 flex flex-col gap-4">
          ...
          >
            ...
            <div className="relative z-10 flex items-center gap-2 px-5 py-4 text-xs lg:text-sm">
              ...
              <div>
                <span className="text-muted-foreground">Smart Upload:</span>{" "}
                <span className="text-accent-foreground">
                  Upload photos and create client gallery automatically
                </span>
              </div>
            </div>
          </motion.div>
          <h2 className="text-4xxl leading-tight tracking-normal md:text-5xl text-primary">
            Streamlined workflow from <br className="hidden md:block" />
            <span className="text-gradient">upload to delivery</span>
          </h2>
          <p className="text-muted-foreground mt-2 max-w-xl text-lg leading-snug">
            Upload your photos and watch as they are automatically optimized,
            watermarked, organized into beautiful galleries, and securely
            delivered to your clients - all in minutes.
          </p>
        </div>
              ...
              >
                {card.isGenerating ? (
                  <div className="relative z-10 flex items-center gap-1.5">
                    {card.icon ? (
                      <card.icon className={`size-3 ${card.iconColor}`} />
                    ) : (
        ...
        <div className="mt-6 grid grid-cols-3 justify-between gap-6">
          {stats.map((stat, index) => (
            <div key={index} className="space-y-1">
              <div className="text-2xl font-bold md:text-3xl text-primary">
                <NumberTicker
                  value={stat.value}
